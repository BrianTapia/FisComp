PROGRAM P2A
	...
	INTEGER::N, i, j
	REAL(8)::V1, V2, h, eps
	REAL(8)::L1(4), L2, s(4), z(4), C(4)
	REAL(8),ALLOCATABLE::V(:, :)
	LOGICAL,ALLOCATABLE::es_conductor(:, :)

	! Largos de la barra superior. Iteraremos sobre estos valores
	! para resolver el inciso a.
	L1 = (/25.d0, 50.d0, 100.d0, 200.d0/)
	...
	! Main:
	call red_inicial(N, V1, V2, V, es_conductor)
	call Poisson(N, eps, es_conductor, V)
	! Notamos que hasta este punto la información de L1 aún no es relevante.

	! Guarda los datos para el inciso A.
	do i = 1, 4
		h = L1(i)/N
		call guardar(N, L1(i), h, V)

		! Para el inciso B:
		call Q(N, L1(i), h, V, s(i), z(i))

		write(6, *)'L1 = ', L1(i)
		write(6, *)'Carga Q en L1: ', s(i)
		write(6, *)'Carga Q en L2: ', z(i)
		write(6, *)'Capacitancia : ', (abs(s(i))+abs(z(i)))/(2*(V1-V2))
		write(6, *)''
	enddo
